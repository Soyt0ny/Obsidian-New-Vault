---
tags:
  - area
  
  
status: active # active, maintenance, archived
priority: high # high, medium, low
review_cadence: monthly # weekly, monthly, quarterly
tech:
  - Python
  - C
domain: Algorithms
---

# Area: ALGORITHMS

## Purpose & Standard
> **Standard:** Mantenerme haciendo problemas de algoritmos para poder mejorar mis capacidades como programador
> *Focus:* Algoritmia y ML

## Current Focus
- [ ] Completar los videos de Abdul
- [ ] Leer Libro [[Algorithms Ilustrado Grokking.pdf]]
- [ ] Hacer 100 ejercicios en leetcode

## Active Projects
*Projects currently moving this area forward.*
- [[LeetCode Problems]]
- [[Aprendizaje de Algoritmos]]

## Routines & Habits
- **Daily:** Hacer por lo menos 2 problemas fáciles, leer 5 paginas de un libro relacionado
- **Weekly:** Hacer un problema medio
- **Monthly:** Hacer un problema difícil

## Knowledge & Resources

##### Videos
- [Lista de Preproducción Abdul Bari](https://www.youtube.com/watch?v=0IAPZzGSbME&list=PLDN4rrl48XKpZkf03iYFl-O29szjTrs_O&index=1)
- [Lista de Preproducción William Fiset](https://www.youtube.com/watch?v=Qmt0QwzEmh0&list=PLDV1Zeh2NRsB6SWUrDFW2RmDotAfPbeHu)
##### Problemas
- [RoadMap Neet Code](https://neetcode.io/roadmap)
- [LeetCode 150 Problemas](https://leetcode.com/studyplan/top-interview-150/)
- [LeetCode](https://leetcode.com/)
##### Documentos
- [Algoritmos Python](https://runestone.academy/ns/books/published/pythonds/index.html)
- [[Algorithms Ilustrado Grokking.pdf]]
- [[Introduction To Algorithms Pro.pdf]]
- [[Cracking the Coding Interview.pdf]]
- [[The Algorithm Design Manual by Steven S. Skiena.pdf]]

## Log / Updates
- **YYYY-MM-DD:** Review notes...
