...
- **Canvas**: 4096×4096 px @ 300 DPI (resolución de grabado de producción)
...
```
Canvas Procreate (4096px, sin pérdida)
    │
    ├──► R2: fuente resolución completa (4096px PNG) — nunca servida a clientes
    │
...
| 3 | **Pipeline Procreate→web** — Bocetos a mano tienen canvases, DPI y transparencias inconsistentes. Export batch manual y propenso a errores. Zonas desalineadas. | **Media (50%)** | **Alto** | Spec rígida de template: 2048×1536, 300 DPI, PNG transparencia, capa plana. Script de normalización (sharp) en upload. Visual diff en admin. Documento iPad export workflow (1-pager PDF en el repo). |
